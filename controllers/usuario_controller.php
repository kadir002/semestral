...
class usuario_controller
{
    ...
    public static function insertar()
    {
        if ($_POST) {
            ...
            $usuarioRegister = $obj->validateUser();
            $EmailRegister = $obj->validateEmail();
            ...
            empty($usuarioRegister ? $error[5] = "El usuario esta registrado" : "");
            empty($EmailRegister ? $error[6] = "El Correo ya existe" : "");
            if (isset($error)) {
    ...
    public static function modificar()
    {
        if ($_POST) {
            ...
            if (isset($error)) {
                ...
                $obj->setNombre_restaurante($nombre_empresa);
                $obj->setCuenta_paypal($cuenta_paypal);
                $obj->setLogo_empresa($imageFondo);
                $obj->setImagen_fondo($foto);
    ...
    public static function view_paymet()
    {

    
        $objUsuario  = new usuario();
        $data=$_SESSION['usuario'];
        $objUsuario->setId($data);

       $result= $objUsuario->getPaymetMet();
      
        require_once("views/template/header.php");
        require_once("views/usuario/viewpaymet.php");
        require_once("views/template/footer.php");
    }
}
